...
wk_end_thu_beg_dt =' '
wk_end_thu_end_dt=' '
dates=((WE_END_THU['DATE']>=wk_end_thu_beg_dt)&(WE_END_THU['DATE']<=wk_end_thu_end_dt))
...
weekenddates1[(weekenddates1['wk_end_thu_end_yr_nu ']>=minlast)&(weekenddates1['wk_end_thu_end_yr_nu ']<=maxlast)]
...
weekenddates3=pd.merge(weekenddates2 , weekenddates1c ,how='inner',left_on=['wk_end_thu_wk_nu','wk_end_thu_end_yr_nu'],right_on=['wk_end_thu_wk_nu','lastyear'])
print(weekenddates3)
...
LCAT_MENU_ITM_PRC_SNAP.sort_values(by=['MCD_GBAL_LCAT_ID_NU','SLD_MENU_ITM_ID','prc_eff_dt'])

PriceSnap1=LCAT_MENU_ITM_PRC_SNAP[['MCD_GBAL_LCAT_ID_NU','SLD_MENU_ITM_ID','prc_eff_dt','prc_end_dt','sld_menu_itm_actv_fl','frnt_cter_itm_prc_am']]
...
OLDPriceSnap1=LCAT_MENU_ITM_PRC_SNAP[['MCD_GBAL_LCAT_ID_NU','SLD_MENU_ITM_ID','prc_eff_dt','prc_end_dt','sld_menu_itm_actv_fl','frnt_cter_itm_prc_am']]

print(OLDPriceSnap1)


PriceSnap2=pd.concat(['PriceSnap1 ','OLDPriceSnap1'])
PriceSnap2['prc_end_dt ']=np.where(PriceSnap2['prc_end_dt '].isna(),"31DEC2099")
...
PriceSnap5=PriceSnap4a['rcom_prc '].isna().PriceSnap4a['rcom_prc']==frnt_cter_itm_prc_am
...
PriceSnap5.sort_values(by=['MCD_GBAL_LCAT_ID_NU ','sld_menu_itm_id'],ascending=False)
...
PriceSnap7['frnt_cter_itm_prc_am']=np.where(PriceSnap7['frnt_cter_itm_prc_am'].apply(np.ceil)>(PriceSnap7['oldPrice'].apply(np.ceil)),increase=1)
...
# 	else if round(frnt_cter_itm_prc_am,.01) < round(oldPrice,.01) then decrease=1;          ##DOUBT
# run;

PriceSnap=PriceSnap.groupby(['increase' 'nochange' 'decrease']).value_counts()  #Doubt
...
LCAT_MENU_ITM_PRC_SNAP.sort_values(by=['MCD_GBAL_LCAT_ID_NU','SLD_MENU_ITM_ID','prc_eff_dt'],inplace=True)
LCAT_MENU_ITM_PRC_SNAP=LCAT_MENU_ITM_PRC_SNAP[['MCD_GBAL_LCAT_ID_NU','SLD_MENU_ITM_ID','prc_eff_dt','prc_end_dt','sld_menu_itm_actv_fl','frnt_cter_itm_prc_am']]
PriceSnapMB1=LCAT_MENU_ITM_PRC_SNAP


PriceSnapMB['prc_end_dt']=np.where(PriceSnapMB1['prc_end_dt'].isna(), '31DEC2099')

wkly_pmix1=pd.DataFrame(f"{mylib1}/wkly_pmix1")
wkly_pmix1['period']=[wkly_pmix1['period']=='LY']
wkly_pmix1=wkly_pmix1.sort_values(by=['mcd_gbal_lcat_id_nu'])
wkly_pmix1=wkly_pmix1.groupby(['mcd_gbal_lcat_id_nu']).agg({'WK_END_THU_ID_NU':np.size(),'WK_END_THU_ID_NU':pd.Series.nunique})
...
storelst=pd.merge(lastyearstrs, thisyearstrs,how='inner',on=['mcd_gbal_lcat_id_nu']).merge(thisyearstrs2,how='inner',on=['mcd_gbal_lcat_id_nu'])
print(storelst)
...
wkly_pmixMB=wkly_pmixMB[(wkly_pmixMB['cal_dt']>=prc_eff_dt) & (wkly_pmixMB['prc_end_dt']<=prc_eff_dt)]
print(wkly_pmixMB)
...
wkly_pmixMB['Menu_sales']= \
wkly_pmixMB[wkly_pmixMB['frnt_cter_itm_prc_am']*wkly_pmixMB['alacarte_units']]
