...
def TAFiles():
    ...
    newfile2 = (
        newfile2[
            ((newfile2['sld_menu_itm_id']) == newfile2['Independent_item']
            & (newfile2['sld_menu_itm_id'] == newfile2['prcg_engn_curr_prc'])) != newfile2['rcom_prc']]
    )
    newfile2 = newfile2[['sld_menu_itm_id',
                        'mcd_gbal_lcat_id_nu',
                        'prcg_engn_curr_prc',
                        'rcom_prc',
                        'frm_engn_rcom_flg',
    ...
    newfile2['prcg_engn_curr_net_prc'] = round(newfile2['prcg_engn_curr_net_prc'],2)
    newfile2['rcom_net_prc'] = round(newfile2['rcom_net_prc'],2)
    recfile2=newfile2

    gcelas2 = pd.read_sas(f"{engine}{file['gcpric2']}")
    merge_data = newfile2.merge( gcelas2, how='left',on='mcd_gbal_lcat_id_nu')
...
def DltFiles():
    recfile1 = pd.DataFrame(f"{engine}{file['newfile1']}")
    recfile1[['sld_menu_itm_id', 'mcd_gbal_lcat_id_nu', 'prcg_engn_curr_prc', 'rcom_prc', 'frm_engn_rcom_flg', 'spr_de_elstc_coef']]
    recfile1['prcg_engn_curr_net_prc'] = (recfile1['prcg_engn_curr_prc'])/(1+file['tax'])
    recfile1['rcom_net_prc']=np.where(
        ~(recfile1['rcom_prc'].isna()),
        round(recfile1['rcom_prc']/(1+file['tax']),2)
    )
    gcelas1=pd.read_sas((f"{engine}{file['gcpric1']}"))
    newfile1 = pd.DataFrame(f"{engine}{file['newfile1']}")
    ...
    gcelas1[['mcd_gbal_lcat_id_nu','gc_elastic','InfluencePrcGC']].unique()


    elasfile1 = pd.read_sas(f"{engine}{file['newfile1']}")
    ...
    return recfile1, gcelas1, elasfile1
...
def combineloop(Vendor):
    ...
    if Vendor.upper() == "COMBINED":
        ...
        recfile1 = pd.read_sas(f'{engine}{file["recfile1"]}', encoding='ISO-8859-1')
        ...
        recfile3 = pd.merge(recfile1, recfile2,on="mcd_gbal_lcat_id_nu", how="left")

        recfile = recfile3.append(recfile2_df)

        gcelas1 = pd.read_sas(f"{engine}{file['gcelas1']}",format='sas7bdat', encoding='utf-8')

        gcelas1.sort_values(by="mcd_gbal_lcat_id_nu")

        gcelas2 = pd.read_sas(f"{engine}{file['gcelas2']}",format='sas7bdat', encoding='utf-8')
        ...
        elasfile1 = pd.read_sas(f"{engine}{file['elasfile1']}", format="sas7bdat", encoding='utf-8')

        elasfile1.sort_values(by=["mcd_gbal_lcat_id_nu", "sld_menu_itm_id", "cross_elastic_item"])

        elasfile2 = pd.read_sas(f"{engine}{file['elasfile2']}", format="sas7dbat", encoding='utf-8')

        elasfile2.sort_values(by=["mcd_gbal_lcat_id_nu", "sld_menu_itm_id", "cross_elastic_item"])

        elasfile3 = pd.merge(elasfile1, elasfile2,on="mcd_gbal_lcat_id_nu", how="left")

        elasfile = elasfile3_merge.append(elasfile2)
...
store_info = pd.read_sas(f"{engine}store_info_sociodemographic_data")
ZoneXField = pd.read_sas(f"{engine}{ZoneXField}")
regions2 = pd.merge(regions1, store_info, how='left', left_on='mcd_gbal_lcat_id_nu',\
                    right_on='global_id').merge(ZoneXField, how='left', left_on='region', right_on='field')
regions2.sort_values(by='mcd_gbal_lcat_id_nu')[['mcd_gbal_lcat_id_nu', 'rest_owsh_typ_id','ops_lvl3_na', 'market', 'zone', 'field']]
...
ItemsNames.sort_values(by=['sld_menu_itm_id']).drop_duplicates(subset='sld_menu_itm_id')
...
itemInfo['exclude']=np.where(
    itemInfo['itm_hier_lvl3'].isin(['Not Used', 'Other No Food'])
    | itemInfo['itm_hier_lvl3'].isin(['Unknown', 'delete']),
    1,0
)
itemInfo['ScrCategory']=np.where(
    itemInfo['itm_hier_lvl3'].isin(['Not Used', 'Other No Food'])
    | itemInfo['itm_hier_lvl3'].isin(['Unknown', 'delete']),
    'Exclude',itemInfo['ITM_HIER_LVL3']
)
...
itemInfo.loc[itemInfo['ITM_HIER_LVL3'].isin(['EVM Large', 'EVM Medium':'ITM_HIER_LVL3', 'Easy Menu']), 'newCategory'] = 'COMBO MEALS'
...
itemInfo.loc[~itemInfo['ITM_HIER_LVL3'].isin(['Drinks', 'ALC Entrees', 'Breakfast', 'EVM Large', 'EVM Medium',
