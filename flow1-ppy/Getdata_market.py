...
def TAFiles():
    ...
    gcelas2 =pd.read_sql( '''
        ...
        ,engine ,params={'newfile2':file['newfile2'],'gcpric2':file['gcpric2']})
...
def combineloop(Vendor):
    ...
    if Vendor.upper() == "COMBINED":
        ...
        gcelas1 = pd.read_sas(f"{engine}gcelas1", format='sas7bdat', encoding='utf-8')

        gcelas1.sort_values(by="mcd_gbal_lcat_id_nu")

        gcelas2 = pd.read_sas(f"{engine}gcelas2", format='sas7bdat', encoding='utf-8')
        ...
        elasfile1_sort = pd.read_sas(f"{engine}elasfile1", format="sas7bdat", encoding='utf-8')

        elasfile1_sort.sort_values(
            by=["mcd_gbal_lcat_id_nu", "sld_menu_itm_id", "cross_elastic_item"])

        elasfile2_sort = pd.read_sas(
            f"{engine}/elasfile2", format="sas7dbat", encoding='utf-8')
    ...
    if Vendor.upper() == "TIGER":
        recfile = pd.read_sas(
            f"{engine}recfile2", format='sas7dbat', encoding='utd-8')

        gcelas = pd.read_sas(f"{engine}gcelas2", format='sas7dbat', encoding='utd-8')

        elasfile = pd.read_sas(f"{engine}/elasfile2",format='sas7dbat', encoding='utd-8')

        return recfile,gcelas,elasfile

    if Vendor.upper() == "DELOITTE":
        recfile = pd.read_sas(
            f"{engine}recfile1", format='sas7dbat', encoding='utd-8')
        gcelas = pd.read_sas(
            f"{engine}gcelas1", format='sas7dbat', encoding='utd-8')
        elasfile = pd.read_sas(f"{engine}elasfile1",
...
# data &mylib3..price_recs_round&round.;
# 	set recfile;
# run;
# data &mylib3..Elasticity_round&round.;       ( #Barath)
# 	set elasfile;
# run;
# data &mylib3..GCElasticity_round&round.;
# 	set gcelas;
# run;
...
itemInfo.loc[~itemInfo['ITM_HIER_LVL3']==['Drinks','ALC Entrees','Breakfast','EVM Large', 'EVM Medium', 'Easy Menu','Salads','Desserts','Happy Meal','McCafe','Fries'] ,'newCategory']='OTHER'
